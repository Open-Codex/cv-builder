...
const ResetDropdown: React.FC<{
  onReset: () => void;
  onLoadShowcase: () => void;
}> = ({ onReset, onLoadShowcase }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      <div
        className={`absolute top-full right-0 mt-[4px] w-[160px] bg-[#2d2d2d] border border-[#404040] rounded-lg shadow-xl z-50 py-[4px] transition-all duration-100 origin-top ${open ? "opacity-100 scale-100 pointer-events-auto" : "opacity-0 scale-95 pointer-events-none"}`}
      >
        <button
          onClick={() => {
            onReset();
...
export const Toolbar: React.FC<ToolbarProps> = ({
  ...
  const aboutCloseRef = React.useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
    ...
    <>
      ...
      <div className="flex items-center gap-[5px] bg-[#1e1e1e] px-[8px] py-[5px] rounded border border-[#404040] w-full lg:w-auto">
        <label
          ...
          className="text-[12.5px] font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap"
        >
          {t("toolbar.theme")}:
        </label>
        <select
        ...
        >
          {THEMES.map((t) => (
            <option key={t} value={t} className="bg-[#1e1e1e] text-[12.5px]">
  ...
  return (
    <div className="relative z-50 min-h-[45px] bg-[#2d2d2d] flex flex-wrap items-center px-[10px] py-[3px] border-b border-[#404040] text-gray-300 shrink-0 gap-[8px]">
      ...
      <div
        className="bg-[#1a1a2e] rounded px-[10px] py-[3px] max-w-[275px] min-w-[200px] shrink-0 hidden sm:block relative"
        data-testid="branding-box"
        ref={errorRef}
      >
        {error ? (
          <div className="flex items-center justify-between gap-1 overflow-hidden">
            <p
              className="text-[11.25px] leading-tight truncate"
            ...
            <button
              ...
              className="text-[11.25px] text-[#ef4444] hover:text-red-400 underline whitespace-nowrap shrink-0"
            >
              {t("error.see_more")}
            </button>
            <div
              className={`absolute top-full left-0 mt-[5px] w-[300px] bg-[#1a1a2e] border border-[#ef4444] p-[10px] rounded shadow-lg z-50 transition-all duration-100 origin-top ${showFullError ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
            >
              <p className="text-[12.5px] text-[#ef4444] font-mono whitespace-pre-wrap break-all">
                {error}
              </p>
              <div className="flex justify-end mt-[5px]">
                <button
                  onClick={() => setShowFullError(false)}
                  className="text-[11.25px] text-gray-400 hover:text-white underline"
                >
                  {t("error.hide")}
                </button>
              </div>
          ...
          <p
            className="text-[11.25px] leading-tight"
            style={{ color: brandingColor }}
          >
            {brandingText.includes("—") ? (
              <>
                <strong>{brandingText.split("—")[0].trim()}</strong> —{" "}
                {brandingText
                  .split("—")
                  .slice(1)
                  .join("—")
      ...
      {/* Primary controls */}
      <div className="hidden lg:contents">{primaryControls}</div>

      {/* "What is CV Builder?" — hover tooltip, screen-centered, wide screens only */}
      <div
        className="hidden xl:block absolute left-1/2 -translate-x-1/2 pointer-events-none"
        style={{ zIndex: 5 }}
      >
        <div
          className="relative pointer-events-auto"
          onMouseEnter={() => {
            if (aboutCloseRef.current) clearTimeout(aboutCloseRef.current);
            setShowAbout(true);
          }}
          onMouseLeave={() => {
            aboutCloseRef.current = setTimeout(() => setShowAbout(false), 200);
          }}
        >
          ...
          <div
            className={`absolute left-1/2 -translate-x-1/2 top-full mt-[6px] w-[360px] max-h-[80vh] overflow-y-auto bg-[#1a1a2e] border border-[#404040] p-[14px] rounded shadow-lg z-50 transition-all duration-150 origin-top ${showAbout ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
          >
            <AboutPanel />
          </div>
        </div>
      ...
      <div className="lg:hidden flex items-center gap-[8px] ml-auto shrink-0">
        ...
        <>
          <div
            className={`fixed inset-0 z-40 transition-opacity duration-100 ${mobileMenuOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
            onClick={() => setMobileMenuOpen(false)}
          />
          <div
            className={`absolute top-full right-2 mt-1 w-72 bg-[#2d2d2d] border border-[#404040] rounded-lg shadow-xl p-3 flex flex-col gap-2 z-50 transition-all duration-100 origin-top-right ${mobileMenuOpen ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
          >
            {primaryControls}
