...
/**
 * Comprehensive E2E tests for YaCV language switching.
 *
 * Covers:
...
async function getLangLabel(page: Page): Promise<string> {
  return (await page.locator('[data-testid="language-label"]').textContent()) ?? '';
}
...
async function editorContains(page: Page, text: string, timeout = 8000) {
  await expect(page.locator('.view-lines').first()).toContainText(text, { timeout });
}
...
test.describe('1 — UI / Selector synchronization', () => {
  test('English mode: toolbar labels and selector are in English', async ({ page }) => {
  ...
  test('Toggle to Spanish: toolbar labels and selector switch to Spanish', async ({ page }) => {
    await page.setViewportSize({ width: 1400, height: 900 });
    await gotoCleanEnglish(page);
  ...
  test('Both localStorage keys stay in sync after toggle', async ({ page }) => {
    ...
    const uiLang = await page.evaluate(() => localStorage.getItem('rendercv-ui-lang'));
    const themeLang = await page.evaluate(() => localStorage.getItem('rendercv-theme-lang'));
...
test.describe('3 — YAML keys change on language toggle', () => {
  ...
  test('Section names translate (Experience ↔ Experiencia)', async ({ page }) => {
    await gotoCleanEnglish(page);
  ...
  test('All entry-level keys translate (company, position, date, etc.)', async ({ page }) => {
    await gotoCleanEnglish(page);
...
test.describe('4 — Conditional value translation', () => {
  test('Unmodified demo CV: full template swap on language change', async ({ page }) => {
    await gotoCleanEnglish(page);
  ...
  test('Unmodified demo CV: full swap back preserves demo content', async ({ page }) => {
    await gotoCleanEnglish(page);
    await toggleLanguage(page); // → ES
    await toggleLanguage(page); // → EN
  ...
  test('Reset-like skeleton: keys translate on language change', async ({ page }) => {
    await page.setViewportSize({ width: 1400, height: 900 });
    await gotoCleanEnglish(page);
...
design:
  ...
  test('User-modified CV: keys translate but user values are preserved', async ({ page }) => {
    await gotoCleanEnglish(page);
...
design:
  ...
  test('Round-trip on user-modified CV: keys return to English, values preserved', async ({ page }) => {
    await gotoCleanEnglish(page);
...
test.describe('5 — Edge cases', () => {
  ...
  test('Language toggle updates page title', async ({ page }) => {
    ...
    expect(enTitle).toContain('YaCV');

    await toggleLanguage(page);
    const esTitle = await page.title();
    expect(esTitle).toContain('YaCV');
    // Titles differ between languages
    expect(esTitle).not.toBe(enTitle);
  });

  test('Multiple toggles on user-modified CV preserve content integrity', async ({ page }) => {
    await gotoCleanEnglish(page);
...
design:
  ...
  test('Starting fresh with Spanish browser: initial state is correct', async ({ page }) => {
  ...
  test('design block is never translated (keys stay in English)', async ({ page }) => {
    await gotoCleanEnglish(page);
  ...
  test('social_networks block is preserved during translation', async ({ page }) => {
    await gotoCleanEnglish(page);
  ...
  test('Empty editor state does not crash on language toggle', async ({ page }) => {
    await gotoCleanEnglish(page);
  ...
  test('Accent colors persist through language toggle', async ({ page }) => {
    ...
    const colorBefore = await page.evaluate(() => localStorage.getItem('rendercv-accent'));

    await toggleLanguage(page);
    await toggleLanguage(page);

    const colorAfter = await page.evaluate(() => localStorage.getItem('rendercv-accent'));
